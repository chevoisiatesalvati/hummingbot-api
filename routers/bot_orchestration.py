    ...
    if response.get("success"):
        try:
            ...
            async with db_manager.get_session_context() as session:
                ...
                logger.info(
                    f"Updated bot run status to STOPPED for {action.bot_name}")
...
async def get_bot_run_by_id(
    ...
    try:
        async with db_manager.get_session_context() as session:
            ...
            if not bot_run:
                raise HTTPException(
                    status_code=404, detail=f"Bot run {bot_run_id} not found")
...
async def _background_stop_and_archive(
    ...
    try:
        ...
        final_status = None
        try:
            final_status = bots_manager.get_bot_status(
                bot_name_for_orchestrator)
            logger.info(
                f"Captured final status for {bot_name_for_orchestrator}: {final_status}")
        except Exception as e:
            logger.warning(
                f"Failed to capture final status for {bot_name_for_orchestrator}: {e}")
        ...
        try:
            async with db_manager.get_session_context() as session:
                ...
                logger.info(
                    f"Updated bot run with stopped_at timestamp and final status for {bot_name}")
        except Exception as e:
        ...
        logger.info(
            f"Stopping bot trading process for {bot_name_for_orchestrator}")
        ...
        if not stop_response or not stop_response.get("success", False):
            error_msg = stop_response.get(
                'error', 'Unknown error') if stop_response else 'No response from bot orchestrator'
            logger.error(f"Failed to stop bot process: {error_msg}")
            return

        # Step 4: Wait for graceful shutdown (15 seconds as requested)
        logger.info(
            f"Waiting 15 seconds for bot {bot_name} to gracefully shutdown")
        ...
        for i in range(max_retries):
            logger.info(
                f"Attempting to stop container {container_name} (attempt {i+1}/{max_retries})")
        ...
        if not container_stopped:
            logger.error(
                f"Failed to stop container {container_name} after {max_retries} attempts")
            return
        ...
        remove_response = docker_manager.remove_container(
            container_name, force=False)

        if not remove_response.get("success"):
            # If graceful remove fails, try force remove
            logging.warning(
                "Graceful container removal failed, attempting force removal")
            remove_response = docker_manager.remove_container(
                container_name, force=True)

        if remove_response.get("success"):
            logging.info(
                f"Successfully completed stop-and-archive for bot {bot_name}")
            ...
            try:
                async with db_manager.get_session_context() as session:
                    ...
                    logger.info(
                        f"Updated bot run deployment status to ARCHIVED for {bot_name}")
            except Exception as e:
            ...
            try:
                async with db_manager.get_session_context() as session:
                    ...
                    logger.info(
                        f"Updated bot run with error status for {bot_name}")
            except Exception as e:
                logger.error(f"Failed to update bot run with error: {e}")

    except Exception as e:
        logging.error(
            f"Error in background stop-and-archive for {bot_name}: {str(e)}")
        ...
        if bot_name_for_orchestrator in bots_manager.active_bots:
            ...
            logger.info(
                f"Removed bot {bot_name_for_orchestrator} from active_bots and cleared MQTT data")
...
async def stop_and_archive_bot(
    ...
    try:
        ...
        logging.info(
            f"Normalized bot_name: {actual_bot_name}, container_name: {container_name}")

        # Step 2: Validate bot exists in active bots
        active_bots = list(bots_manager.active_bots.keys())

        # Check if bot exists in active bots (could be stored as either format)
        bot_found = (actual_bot_name in active_bots) or (
            container_name in active_bots)
...
async def deploy_v2_controllers(
    ...
    try:
        ...
        if response.get("success"):
            ...
            # Copy controller configs from global location to instance directory
            # This ensures the Docker container can access them at /home/hummingbot/conf/controllers
            instance_controllers_dir = f"instances/{unique_instance_name}/conf/controllers"
            global_controllers_dir = "conf/controllers"

            # Ensure instance controllers directory exists
            fs_util.create_folder(
                f"instances/{unique_instance_name}/conf", "controllers")

            # Copy each controller config file
            for controller_config_file in controllers_with_extension:
                try:
                    global_config_path = f"{global_controllers_dir}/{controller_config_file}"
                    instance_config_path = f"{instance_controllers_dir}/{controller_config_file}"

                    if fs_util.path_exists(global_config_path):
                        fs_util.copy_file(global_config_path,
                                          instance_config_path)
                        logger.info(
                            f"Copied controller config '{controller_config_file}' to instance directory")
                    else:
                        logger.warning(
                            f"Controller config '{controller_config_file}' not found in global location, skipping copy")
                except Exception as e:
                    logger.error(
                        f"Failed to copy controller config '{controller_config_file}': {e}")
            ...
            try:
                async with db_manager.get_session_context() as session:
                    ...
                    logger.info(
                        f"Created bot run record for controller deployment {unique_instance_name}")
            except Exception as e:
                logger.error(f"Failed to create bot run record: {e}")
                # Don't fail the deployment if bot run creation fails
